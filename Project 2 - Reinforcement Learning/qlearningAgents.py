...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in actions:
            values = values + [self.getQValue(state, action)]
    ...
    def update(self, state, action, nextState, reward):
        ...
        if len(actions) > 0:
            reward = reward + self.discount * max([self.getQValue(nextState, next_action) for next_action in actions])

        second_part = self.alpha * reward
