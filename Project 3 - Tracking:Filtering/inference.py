...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = self.legalPositions
        for oldPos in allPossible:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for pos, prob in newPosDist.items():
                "belief_t(s) <-- sum_s_t-1( P(x_t = s | x_t-1 = s_t-1) * belief_t-1(x_t-1 = s_t-1) )"
                newBelief[pos] += prob * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "Place particles evenly in each state until the limit is reached"
        counter = 0
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            allPossible = util.Counter()
            "Compute the weight based on the Transition Model"
            oldBelief = self.getBeliefDistribution()
            for pos in self.legalPositions:
                trueDistance = util.manhattanDistance(pacmanPosition, pos)
                allPossible[pos] = emissionModel[trueDistance] * oldBelief[pos]

            "Check if the particles have a nonzero weight sum"
            if not any(allPossible.values()):
                "If not, reinitialize all the particles uniformly"
                self.initializeUniformly(gameState)
            else:
                "If so, resample all the particles and replace self.particles"
                resampledParticles = []
    ...
    def elapseTime(self, gameState):
        ...
        "Create a new list of particles by sampling from the updated position distribution for each particle"
        for particle in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
            resampledParticles.append(util.sample(newPosDist))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        weight with each position) is incorrect and may produce errors.
        """
        "Build the list of possible positions for each ghost and randomize it"
        positions = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(positions)

        "Initialize the particles uniformly based on those positions"
        particles = []
        counter = 0
        while counter < self.numParticles:
    ...
    def observeState(self, gameState):
        ...
        "Weigh the particles based on the Transition Model"
        allPossible = util.Counter()
        for particle in self.particles:
            prob = 1.0
            for index in range(self.numGhosts):
                if noisyDistances[index] is None:
                    ...
                    trueDistance = util.manhattanDistance(pacmanPosition, particle[index])
                    prob *= emissionModels[index][trueDistance]
            allPossible[particle] += prob

        "Check if the particles have a nonzero weight"
        if not any(allPossible.values()):
            "Reinitialize the particles uniformly and update the particles of ghosts in jail"
            self.initializeParticles()
            for particle in self.particles:
                for index in range(self.numGhosts):
                    if noisyDistances[index] is None:
            ...
            "Normalize and sample the distribution to get a new list of particles"
            resampledParticles = []
            allPossible.normalize()
            for index in range(0, self.numParticles):
                resampledParticles.append(util.sample(allPossible))
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "For each ghost, get the new Sensor Model and update the position of that ghost in the newParticle"
            for index in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle),
                                                             index, self.ghostAgents[index])
                newParticle[index] = util.sample(newPosDist)

            resampledParticles.append(tuple(newParticle))
        self.particles = resampledParticles
